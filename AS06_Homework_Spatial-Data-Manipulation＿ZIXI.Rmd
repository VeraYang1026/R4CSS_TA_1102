...
sf <- sf_tpe %>%
  mutate(f_per = round(f_cnt / p_cnt,digits = 3),
         p_ranking = order((p_cnt), decreasing = T),
         p_ranking = order(p_ranking),
         round(f_per, digits = 3)) %>%
  arrange(f_per, decreasing = TRUE) %>%
  tibble() %>%
  select(county,town,village,f_per,p_ranking,p_cnt) 

head(arrange(sf,desc(f_per)),5)
...
df_mayor_tpe <- df_mayor_tpe %>%
  select(-4,-5) %>%
  group_by(town) %>%
  spread(key = `姓名`, value = `得票率`) %>%
  select(county,town,`柯文哲`,`丁守中`,`姚文智`) %>%
  ungroup()
...
sf_tpe_join <- full_join(sf_tpe_agg, df_mayor_tpe, by = c("county","town") )  
...
p1 <- sf_tpe_join %>% ggplot(aes(fill = 柯文哲)) + geom_sf(color = NA) +
  scale_fill_gradient(low = "white", high = "#28C8C8") +
  theme_void() +
  theme(panel.grid.major = element_blank(), 
         panel.grid.minor = element_blank()) +
    labs(title = '2018台北市長選舉得票率') +
    theme(text=element_text(family="黑體-繁 中黑")) +
    geom_sf_text(aes(label = `town`),size =2,family="黑體-繁 中黑") +
    theme(legend.position = "bottom")


p2 <-  sf_tpe_join %>% ggplot(aes(fill = 丁守中)) + geom_sf(color = NA) +
  scale_fill_gradient(low = "white", high = "#000095") +
  theme_void() +
  theme(panel.grid.major = element_blank(), 
         panel.grid.minor = element_blank()) +
    labs(title = '2018台北市長選舉得票率') +
...
p3  <- sf_tpe_join %>% ggplot(aes(fill = 姚文智)) + geom_sf(color = NA) +
